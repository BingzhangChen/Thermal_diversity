...
real, parameter  :: offset = 60.
  y   = cos(2. * pi * (day-offset) / dble(d_per_y) )
END FUNCTION cosine

SUBROUTINE BIOLOGY(ALPHAG_)
IMPLICIT NONE
REAL, INTENT(IN):: ALPHAG_
REAL, PARAMETER :: RDN  = 0.1    !Regeneration rate from DET to NO3
REAL, PARAMETER :: gmax = 1.     !Maximal zooplankton ingestion rate
...
REAL, PARAMETER :: mz  = 0.15
REAL, PARAMETER :: Ez  = 0.65
...
do j = 1, NPHY
   ...
   call GROWTH(temp, NO3, par_, Topt(j), KN, aI0, mu, mumax(j))
...
do j = 1, NPHY
   call GROWTH(t_, NO3, par_, Topt(j), KN,  aI0, mu_,  mumax_)
...
SUBROUTINE GROWTH(t, N, par,  Topt_, KN, aI0, gr, um)
IMPLICIT NONE
