library(Rcpp)
sourceCpp('utils.cpp')
#alphaG=2,    high diversity, species can coexist
...
d_per_y <- 365
newx    <- 1:d_per_y
get_data <- function(i,j,k){
  #i: average temp
  #j: Diversity (alphaG)
  #k: amplitude
  sst  <- avgtemp[i]
  file <- paste0('Mod',sst,'_',j,'_',k,'.out')
...
pdf('Fig4finalyear.pdf',width=14,height=8)
par(font.lab  = 1,
    family    = "serif",
    cex.axis  = 1.7,
    cex.lab   = 1.7,
...
for (n in 1:2){  #Am
   for (i in 1:2){  #Mean temp
      ...
      temper <- get_data(i,1,n)$Temp[ff]
   
      YLIM <- range(c(get_data(i,1,n)$NO3[ff], get_data(i,2,n)$NO3[ff]))
      ...
      plot(newx, get_data(i,1,n)$NO3[ff], xaxt='n',
          xlab='',ylab=YLAB[1],
          ylim=YLIM,type = 'l')
      points(newx, get_data(i,2,n)$NO3[ff], type='l', col=2)
      ...
      YLIM <- range(c(get_data(i,1,n)$PHY[ff], get_data(i,2,n)$PHY[ff]))
      plot(newx, get_data(i,1,n)$PHY[ff], xlab='', xaxt='n',
          ylab=YLAB[2],
          ylim=YLIM,type = 'l')
      mtext(let[2],adj=0,cex=1.4)
      points(newx, get_data(i,2,n)$PHY[ff], type='l', col=2)
      axis(side=1, at=dates, label=months)
   
      #Zooplankton biomass plot
      YLIM <- range(c(get_data(i,1,n)$ZOO[ff], get_data(i,2,n)$ZOO[ff]))
      plot(newx, get_data(i,1,n)$ZOO[ff], xlab='', xaxt='n',
          ylab=YLAB[3],
          ylim=YLIM,type = 'l')
      mtext(let[3],adj=0,cex=1.4)
      points(newx, get_data(i,2,n)$ZOO[ff], type='l', col=2)
      axis(side=1, at=dates, label=months)
   }
}
...
#Fig. 5 seasonal variations of mean Topt, sigmaZ, mumax, and Ew
NROW <- 4  #NROW figures
pdf('Fig5avgTopt.pdf',width=14,height=3*NROW)
par(font.lab  = 1,
    family    = "serif",
    cex.axis  = 1.5,
    cex.lab   = 1.5,
...
for (n in 1:2){
  for (i in 1:2){
     ...
     #Mean Topt
     YLIM <- range(c(temper, TKrev(get_data(i,1,n)$avgZ[ff]), 
                             TKrev(get_data(i,2,n)$avgZ[ff])))
     ...
     k <- 1
     plot(newx, TKrev(get_data(i,1,n)$avgZ[ff]), xlab='', xaxt='n',
         ylab=YLAB[k],
         ylim=YLIM,type = 'l')
     mtext(let[k],adj=0,cex=1.5)
     points(newx, TKrev(get_data(i,2,n)$avgZ[ff]), type='l', col=2)
     axis(side=1, at=dates, label=months)
    ...
    #SigmaZ
     k <- k + 1
     YLIM <- c(0, 3)
     plot(newx, get_data(i,1,n)$sigmaZ[ff], xlab='', xaxt='n',
         ylab=YLAB[k],
         ylim=YLIM,type = 'l')
  
     mtext(let[k],adj=0,cex=1.5)
     points(newx, get_data(i,2,n)$sigmaZ[ff], type='l', col=2)
     axis(side=1, at=dates, label=months)
  
     #mumax
     k    <- k + 1
     YLIM <- c(.1, 1.5)
     plot(newx, get_data(i,1,n)$mumax[ff], xlab='', xaxt='n',
         ylab=YLAB[k],
         ylim=YLIM,type = 'l')
  
     mtext(let[k],adj=0,cex=1.5)
     points(newx, get_data(i,2,n)$mumax[ff], type='l', col=2)
     axis(side=1, at=dates, label=months)
  
     #Ew
     k <- k + 1
     YLIM <- c(-0.6, 1.1)
     plot(newx, get_data(i,1,n)$Ew[ff], xlab='', xaxt='n',
         ylab=YLAB[k],
         ylim=YLIM,
         type='l')
  
     mtext(let[k],adj=0,cex=1.5)
     points(newx, get_data(i,2,n)$Ew[ff], type='l', col=2)
     abline(h=0.32, lty=2)
     abline(h=0.65, lty=2)
     if (i == 1){
